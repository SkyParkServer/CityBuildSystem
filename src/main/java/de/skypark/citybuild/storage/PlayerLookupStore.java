...
public class PlayerLookupStore {
  ...
  public UUID findUuidByName(String name) {
    ...
    try (Connection c = data.db().getConnection();
        PreparedStatement ps =
            c.prepareStatement(
                "SELECT uuid FROM cb_players WHERE LOWER(last_name) = LOWER(?) ORDER BY last_join DESC LIMIT 1")) {
      ps.setString(1, name);
  ...
  public String findNameByUuid(UUID uuid) {
    try (Connection c = data.db().getConnection();
        PreparedStatement ps =
            c.prepareStatement("SELECT last_name FROM cb_players WHERE uuid = ? LIMIT 1")) {
