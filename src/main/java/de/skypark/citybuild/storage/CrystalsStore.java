...
public class CrystalsStore {
  ...
  public int get(OfflinePlayer target) {
    ...
    try (Connection c = data.db().getConnection();
        PreparedStatement ps =
            c.prepareStatement("SELECT amount FROM cb_crystals WHERE uuid = ?")) {
  ...
  public void set(OfflinePlayer target, int amount) {
    ...
    try (Connection c = data.db().getConnection();
        PreparedStatement ps =
            c.prepareStatement("UPDATE cb_crystals SET amount = ? WHERE uuid = ?")) {
  ...
  private void ensureRow(OfflinePlayer target) {
    try (Connection c = data.db().getConnection();
        PreparedStatement ps =
            c.prepareStatement("INSERT IGNORE INTO cb_crystals (uuid, amount) VALUES (?, 0)")) {
      ps.setString(1, target.getUniqueId().toString());
      ps.executeUpdate();
    } catch (SQLException e) {
      e.printStackTrace();
