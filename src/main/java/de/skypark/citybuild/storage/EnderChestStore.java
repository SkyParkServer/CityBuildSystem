...
public class EnderChestStore {
    ...
    public void save(UUID uuid, Inventory inventory) {
        ...
        try (Connection c = data.db().getConnection();
             PreparedStatement ps = c.prepareStatement("INSERT INTO cb_enderchest (uuid, contents) VALUES (?, ?) ON DUPLICATE KEY UPDATE contents = VALUES(contents)")) {
    ...
    private String getRaw(UUID uuid) {
        try (Connection c = data.db().getConnection();
             PreparedStatement ps = c.prepareStatement("SELECT contents FROM cb_enderchest WHERE uuid = ?")) {
            ps.setString(1, uuid.toString());
