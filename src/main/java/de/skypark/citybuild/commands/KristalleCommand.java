...
public class KristalleCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            plugin.messages().error(sender, "Nutze: /kristalle <Spieler> | /kristalle <give|set|reset> <Spieler> [Betrag]");
            return true;
        }

        if (args.length == 1) {
            OfflinePlayer target = plugin.getServer().getOfflinePlayer(args[0]);
            plugin.messages().message(sender, "&7Kristalle von &e" + target.getName() + "&7: &b" + crystals.get(target));
            return true;
        }
        ...
        if ("reset".equals(action)) {
            ...
            plugin.messages().success(sender, "Kristalle von &e" + target.getName() + "&a wurden zurueckgesetzt.");
            return true;
        }
        ...
        if ("give".equals(action)) {
            ...
            plugin.messages().success(sender, "Es wurden &b" + amount + "&a Kristalle an &e" + target.getName() + "&a gegeben.");
            return true;
        }
        if ("set".equals(action)) {
            crystals.set(target, amount);
            plugin.messages().success(sender, "Kristalle von &e" + target.getName() + "&a wurden auf &b" + Math.max(0, amount) + "&a gesetzt.");
            return true;
        }
