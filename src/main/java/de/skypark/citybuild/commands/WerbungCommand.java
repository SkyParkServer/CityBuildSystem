...
public class WerbungCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (last > 0 && now - last < COOLDOWN_MS) {
            ...
            plugin.messages().error(player, "Du kannst erst in " + hours + "h " + minutes + "m wieder Werbung senden.");
            return true;
        }
        ...
        for (Player online : Bukkit.getOnlinePlayers()) {
            ...
            TextComponent component = new TextComponent("§6§lSkyPark §8» §bTeleportation §7(klickbar zum Plot)");
            component.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/werbungtp " + player.getUniqueId()));
    ...
    private boolean canSendWerbungHere(Player player) {
        if (Bukkit.getPluginManager().getPlugin("PlotSquared") == null) {
            plugin.messages().error(player, "PlotSquared wurde nicht gefunden. Werbung ist derzeit deaktiviert.");
            return false;
        }
        ...
        try {
            ...
            if (plot == null) {
                plugin.messages().error(player, "Du kannst Werbung nur auf deinem oder vertrauten Plot senden.");
                return false;
            }
            ...
            plugin.getLogger().warning("PlotSquared-Pruefung fuer /werbung fehlgeschlagen: " + ex.getMessage());
            plugin.messages().error(player, "Werbung konnte nicht geprueft werden. Versuche es spaeter erneut.");
            return false;
        }
    }
}
